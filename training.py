...
from PIL import Image  # Using PIL for better PNG support
...
# Set CUDA device from environment variable (default to "0" if not set)
...
class ReconstructionHead(nn.Sequential):
    def __init__(self, in_channels, out_channels=1, upsampling=1):
        layers = [
            nn.Conv2d(in_channels, out_channels, kernel_size=3, padding=1),
            nn.Upsample(scale_factor=upsampling, mode="bilinear", align_corners=True),
            nn.Sigmoid(),  # Assuming input image is normalized to [0, 1]
        ]
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x):
        ...
        return segmentation_logits, reconstructed_image
...
def load_split_data(base_dir="dataset_split"):
    data = {}
    for split in ["train", "val", "test"]:
        ...
        data[split] = (image_paths, mask_paths)
...
class BreastDataset(Dataset):
    """Custom Dataset for Breast images and masks."""
    ...
    def __getitem__(self, idx):
        ...
        # Expand dims to match channel count if needed
        ...
        image = image.float()
        mask = mask.float()
        # Ensure both are channel-first tensors
...
# Initialize Network
def get_network(
    vit_name="R50-ViT-B_16", img_size=224, num_classes=1, n_skip=3, vit_patches_size=16
):
    """
    Initializes the Vision Transformer-based segmentation network.
    """
    ...
    net = VisionTransformer(config_vit, img_size=img_size, num_classes=num_classes)
    return net
...
def dice_score(preds, targets, threshold=0.5):
    ...
    preds = (preds > threshold).float()  # Convert probabilities to binary mask
    targets = targets.float()

    intersection = (preds * targets).sum(dim=(1, 2, 3))
    union = (preds + targets).sum(dim=(1, 2, 3))

    dice = (2.0 * intersection) / (union + 1e-8)  # Dice Score Formula
    return dice.mean().item()


def dice_loss(preds, targets):
    """Computes the Dice Loss (with Sigmoid)."""
...
def bce_dice_loss(preds, targets):
    """Computes BCE Loss + Dice Loss (with Sigmoid)."""
    bce = F.binary_cross_entropy_with_logits(preds, targets)  # BCE Loss (with logits)
    dice = dice_loss(preds, targets)  # Dice Loss (already applies sigmoid)
    return bce + dice  # Combined Loss


def reconstruction_loss(pred_recon, original):
    return F.mse_loss(pred_recon, original)


def total_loss_fn(
    seg_preds, seg_targets, recon_preds, original_images, alpha=1.0, beta=0.1
):
    seg_loss = bce_dice_loss(seg_preds, seg_targets)
    recon_loss = reconstruction_loss(recon_preds, original_images)
    return alpha * seg_loss + beta * recon_loss
...
def train_model(
    ...
    """Train the model with a Cosine Annealing Learning Rate Scheduler & WandB logging."""
    net.to(device)
    ...
    for epoch in range(num_epochs):
        ...
        for images, masks in tqdm(
            ...
            optimizer.zero_grad()
            seg_outputs, recon_outputs = net(images)
            total_loss = total_loss_fn(seg_outputs, masks, recon_outputs, images)
            total_loss.backward()
            optimizer.step()

            train_loss += total_loss.item()
            train_dice_score += dice_score(seg_outputs, masks)
        ...
        with torch.no_grad():
            for images, masks in tqdm(
                ...
                val_dice_score += dice_score(seg_outputs, masks)
        ...
        if val_loss < best_val_loss:
            ...
            print(
                f"✅ Model saved at {checkpoint_path} (Best Val Loss: {best_val_loss:.4f})"
            )
        ...
        print(
            f"📉 Train Loss: {train_loss:.4f} | 🏆 Train Dice Score: {train_dice_score:.4f}"
        )
...
train_transform = A.Compose(
    [
        A.Resize(224, 224),  # Resizing to match TransUNet's input
        A.HorizontalFlip(p=0.5),
        A.VerticalFlip(p=0.5),
        A.RandomRotate90(p=0.5),
        # A.ShiftScaleRotate(shift_limit=0.05, scale_limit=0.05, rotate_limit=15, p=0.5),
        # A.RandomBrightnessContrast(p=0.2),
        A.Normalize(mean=(0.5,), std=(0.5,)),  # Grayscale normalize
        ToTensorV2(),
    ]
)

# Only resizing and normalization for validation/test
...
if __name__ == "__main__":
    ...
    wandb.init(
        project="breast-segmentation",
        name=f"breast-segmentation-{EXPERIMENT_NAME}-{TIMESTAMP}",
        config={
            "epochs": EPOCHS,
            "batch_size": 8,
            "optimizer": "Adam",
    ...
    train_dataset = BreastDataset(
        train_imgs,
        train_masks,
        train_transform if ADD_AUGUMENTATIONS else val_transform,
    )
    val_dataset = BreastDataset(val_imgs, val_masks, val_transform)
    ...
    output_dir = os.path.join("trained_models", EXPERIMENT_NAME, TIMESTAMP)
    os.makedirs(output_dir, exist_ok=True)
    checkpoint_path = os.path.join(output_dir, "best_model.pth")
    # Train model with Cosine Scheduler
    train_model(
        net,
